...
class StarNameException(ValueError):
    pass
...
def translate_name(starname):
    ...
    """
    rg = re.match(r"(?P<catalog>\D*)(?P<number>\d+)", starname)
    if rg:
...
class _GCPD:
    ...
    def fetch_data(self, starname, rem):
        ...
        d['mode'] = 'starno'
        query = urllib.parse.urlencode(d) + '&rem=' + rem
        with urllib.request.urlopen(self.GCPD_action + query) as f:
            h = GCPD_parser()
            s = f.read()
            h.feed(s.decode())
...
class GCPD_Photometry_UBV_CAPE(_GCPD):
    """ UBV Cape photometry.
    """
    system_string = 'UBV Cape'
    system_common_name = 'Cape'
    ...
    def process_data(self, d):
        V = [to_float(v) for v in d['V']]
        B = [safe_add(v, bv) for v, bv in zip(V, d['B-V'])]
        U = [safe_add(b, ub) for b, ub in zip(B, d['Uc-B'])]
        return {'V': V, 'B': B, 'U': U}
...
PHOTOMETRY_classes = dict([(translate_photo_name(_class.system_string), _class)
                           ...
                           if re.match('^GCPD_Photometry', name) and inspect.isclass(_class)])
    ...
    try:
        ...
        if target and photosystem:
            ...
            try:
                ...
                print("# The following photometric systems are supported for this star: ", endline='')
                print(", ".join(l))
            except GCPD_No_Data:
